...
class JSONFolderListing(object):
	...
	root_relative = aq_inner(self.context).getPhysicalPath()[len(plone_root.getPhysicalPath()):]       
	for i in range(len(root_relative)):
		...
		if ITranslatable.providedBy(obj):
		  ...
		  if (currentLanguage == 'de') and (obj.hasTranslation('en')):
		    getUrlForLanguageCode = obj.getTranslation('en').absolute_url()
		  if (currentLanguage == 'en') and (obj.hasTranslation('de')):
		    getUrlForLanguageCode = obj.getTranslation('de').absolute_url()
		    
		if not now[-1] == 'talkback':
		    result.append({
			'getUrlForLanguageCode' : getUrlForLanguageCode,
			'getTitleForLanguageCode' : translate(MessageFactory('linguaplone')("legend_existing_translations", default="Existing translations"), context=self.context.REQUEST),
			'currentLanguage' : currentLanguage,
			'getParentFolderURL' : self.context.aq_inner.getParentNode().absolute_url(),
			'getParentFolderTitle': translate(MessageFactory('plone')('go_to_parent_url', default="Up one level"), context=self.context.REQUEST),
		    })
