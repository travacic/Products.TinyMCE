...
BrowserDialog.prototype.parseImageScale = function (url) {
    ...
    if (url.indexOf('/') > -1) {
        ...
        if (last_part.indexOf('image') > -1) {
            parsed.scale = "image_" + parts.pop().substring(6);
...
BrowserDialog.prototype.getFolderListing = function (context_url, method) {
    ...
    jq.ajax({
	    ...
	    // ctravaci_ccc breadcrumbs patch	    
	    html.push('<div id="currentLanguageShortcut">');
            jq.each(data.path, function (i, item) {
                if ((i === len - 1) && (item.getUrlForLanguageCode !== '') && (item.getUrlForLanguageCode !== undefined)) {	
		      if (item.currentLanguage === 'de') { html.push('<img class="langShortcutFlag" width="14" height="11" src="++resource++country-flags/gb.gif" />');}
		      if (item.currentLanguage === 'en') { html.push('<img class="langShortcutFlag" width="14" height="11" src="++resource++country-flags/de.gif" />');}		     
		      html.push('<a href="' + item.getUrlForLanguageCode + '">' + item.getTitleForLanguageCode + '</a>');		                 
                }                 
            });
	    html.push('</div>');
	    
	    html.push('<div id="parentFolderShortcut">');
            jq.each(data.path, function (i, item) {
                if ((i === len - 1) && (item.getParentFolderURL !== undefined)) {
		  html.push('<a href="' + item.getParentFolderURL + '"> <img class="pfIcon" width="14" height="11" src="++resource++plone.resourceeditor/filemanager/images/parentfolder.png" /> ' + item.getParentFolderTitle + '</a>');
                }                 
            });
	    html.push('</div>');
